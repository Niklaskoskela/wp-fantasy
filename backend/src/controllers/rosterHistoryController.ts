    ...
    for (const entry of rosterEntries) {
      if (!entry.playerId || typeof entry.isCaptain !== 'boolean') {
        res
          ...
          .json({
            error:
              'Each roster entry must have playerId and isCaptain properties',
          });
        return;
      }
    }
...
export async function getRosterHistory(
  ...
  try {
    const { teamId, matchDayId } = req.params;
    const rosterHistory = await rosterHistoryService.getRosterHistory(
      teamId,
      matchDayId
    );
 ...
 */
export async function getTeamRosterHistory(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    const rosterHistory =
      await rosterHistoryService.getTeamRosterHistory(teamId);
 ...
 */
export async function getMatchDayRosterHistory(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    const rosterHistory =
      await rosterHistoryService.getMatchDayRosterHistory(matchDayId);
 ...
 */
export async function snapshotAllTeamRosters(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    const snapshots =
      await rosterHistoryService.snapshotAllTeamRosters(matchDayId);
 ...
 */
export async function removeRosterHistory(
  req: Request,
  res: Response
): Promise<void> {
  try {
 ...
 */
export async function checkRosterHistory(
  req: Request,
  res: Response
): Promise<void> {
  try {
    const { teamId, matchDayId } = req.params;
    const exists = await rosterHistoryService.hasRosterHistory(
      teamId,
      matchDayId
    );
    res.json({ exists });
