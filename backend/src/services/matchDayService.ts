...
export async function startMatchDay(matchDayId: string): Promise<boolean> {
  ...
  const { snapshotAllTeamRosters, hasRosterHistory } = await import(
  ...
  if (!alreadySnapshotted) {
    ...
    console.log(
      `Rosters snapshotted for matchday ${matchDay.title} (${matchDayId})`
    );
  } else {
    console.log(
      `Rosters already snapshotted for matchday ${matchDay.title} (${matchDayId})`
    );
  }
