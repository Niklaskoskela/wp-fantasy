...
export async function addPlayerToTeam(
  ...
  if (teamCheck.rows.length === 0) {
    ...
    const teamExistsCheck = await pool.query(
...
export async function getUserTeam(userId: string): Promise<Team | null> {
  const result = await pool.query('SELECT team_id FROM users WHERE id = $1', [
    userId,
  ]);
...
export async function getTeamsWithScores(): Promise<TeamWithScores[]> {
  ...
  const rosterHistoryMap: {
    [teamId: string]: { [matchDayId: string]: RosterHistory[] };
  } = {};
  ...
  const sortedMatchDays = [...allMatchDays].sort(
  ...
  const teamsWithScores = Object.values(teamsMap).map((team) => {
    ...
    for (const matchDay of allMatchDays) {
      ...
      if (!rosterToUse || rosterToUse.length === 0) {
        const pastMatchDays = sortedMatchDays.filter(
          (md) =>
