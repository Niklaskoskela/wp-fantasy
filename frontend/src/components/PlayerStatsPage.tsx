...
import { RosterHistory } from '../../../shared/src/types';
...
export function PlayerStatsPage() {
  ...
  const MatchDayRosterTable = ({
    ...
    const rosterData = useMemo(() => {
      return rosterEntries
        .map((rosterEntry) => {
          ...
          const playerStatsForMatchDay = matchDayPlayersWithStats.find(
            (p) => p.id === rosterEntry.id
          );
    ...
    const totalTeamPoints = rosterData.reduce(
        ...
        <>
          {!myTeam ? (
            <Alert severity='info' sx={{ mb: 2 }}>
              No team found. Please create a team first to view roster
              statistics.
            </Alert>
          ) : isLoadingRoster ? (
            <Box display='flex' justifyContent='center' p={4}>
            ...
            <>
              ...
              {Object.keys(rosterDataByMatchDay).length === 0 ? (
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {Object.entries(rosterDataByMatchDay).map(
                    ([matchDayId, rosterEntries]) => (
                      <MatchDayRosterTable
                        key={matchDayId}
                        matchDayId={matchDayId}
